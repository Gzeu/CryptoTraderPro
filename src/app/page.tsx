...
import { TrendingUp, TrendingDown, RefreshCw, Star, BarChart2, Wallet, Search, Moon, Sun, ChevronUp, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react'
...
function SortIcon({ field, active, dir }: { field: string; active: SortField; dir: SortDir }) {
  ...
  return dir === 'asc' ? <ChevronUp size={11} style={{ color: 'var(--primary)' }} /> : <ChevronDown size={11} style={{ color: 'var(--primary)' }} />
}

export default function DashboardPage() {
  const { data: coins = [], isLoading, refetch, dataUpdatedAt } = useMarketsQuery()
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 shrink-0">
            ...
            <nav className="hidden md:flex items-center gap-1 text-sm">
              <span className="px-3 py-1 rounded-lg font-medium" style={{ background: 'var(--surface-2)', color: 'var(--text)' }}>Dashboard</span>
              <Link href="/watchlist" className="px-3 py-1 rounded-lg hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}>Watchlist</Link>
              <Link href="/portfolio" className="px-3 py-1 rounded-lg hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}>Portfolio</Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-1">
            {lastUpdate && <span className="text-xs num hidden md:block mr-1" style={{ color: 'var(--text-faint)' }}>{lastUpdate.toLocaleTimeString()}</span>}
            <button onClick={() => refetch()} title="Refresh" className="p-2 rounded-lg hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}><RefreshCw size={15} /></button>
            <button onClick={toggleTheme} title="Toggle theme" className="p-2 rounded-lg hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {isLoading
            ...
            : [
                ...
                { label: 'BTC',  icon: <Wallet size={14}/>,   value: fmtPrice(btc?.current_price  ?? 0),  sub: btc  ? fmtPct(btc.price_change_percentage_24h  ?? 0) : undefined, up: (btc?.price_change_percentage_24h  ?? 0) >= 0 },
                { label: 'EGLD', icon: <Star size={14}/>,     value: fmtPrice(egld?.current_price ?? 0),  sub: egld ? fmtPct(egld.price_change_percentage_24h ?? 0) : undefined, up: (egld?.price_change_percentage_24h ?? 0) >= 0 },
              ].map((k, i) => (
                <div key={i} className="rounded-xl p-4 border fade-up" style={{ background: 'var(--surface)', borderColor: 'var(--border)', animationDelay: `${i*60}ms` }}>
                  <div className="flex items-center gap-1.5 mb-1 text-xs" style={{ color: 'var(--text-muted)' }}>{k.icon}{k.label}</div>
                  <p className="font-bold text-lg num leading-none">{k.value}</p>
                  {k.sub && <p className="text-xs num mt-0.5" style={{ color: k.up ? 'var(--green)' : 'var(--red)' }}>{k.sub}</p>}
                </div>
              ))
          }
        </div>
        ...
        <div className="flex gap-1 mb-4 p-1 rounded-xl w-fit" style={{ background: 'var(--surface-2)' }}>
          {(['market','watchlist'] as Tab[]).map(t => (
            <button key={t} onClick={() => { setTab(t); setPage(1) }}
              ...
              style={tab===t ? { background:'var(--surface)', color:'var(--text)', boxShadow:'0 1px 4px rgba(0,0,0,0.08)' } : { color:'var(--text-muted)' }}>
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
          {/* Header row */}
          <div className="grid grid-cols-[2rem_1fr_7rem_7rem_6rem_6rem_5rem_4rem] gap-3 px-4 py-3 text-xs font-medium border-b"
            ...
            <button className="text-left" onClick={() => handleSort('market_cap_rank')}>
              <span className="flex items-center gap-0.5">#<SortIcon field="market_cap_rank" active={sortField} dir={sortDir} /></span>
            </button>
            <span>Name</span>
            {COLS.map(col => (
              <button key={col.field} className={`${col.cls} flex items-center justify-end gap-0.5 w-full`} onClick={() => handleSort(col.field)}>
                {col.label}<SortIcon field={col.field} active={sortField} dir={sortDir} />
              </button>
            ))}
            <span className="text-right hidden md:block">7d</span>
          ...
          {isLoading
            ...
            : paginated.length === 0
              ...
              : paginated.map((coin: Coin, i: number) => (
                  <div key={coin.id}
                    className="grid grid-cols-[2rem_1fr_7rem_7rem_6rem_6rem_5rem_4rem] gap-3 px-4 py-3 border-b items-center hover:opacity-80 transition-opacity fade-up"
                    ...
                    <p className="text-right text-xs num hidden sm:block" style={{ color:'var(--text-muted)' }}>{fmtLarge(coin.market_cap)}</p>
                    <p className="text-right text-xs num hidden lg:block" style={{ color:'var(--text-muted)' }}>{fmtLarge(coin.total_volume)}</p>
                    <div className="hidden md:flex justify-end"><SparklineChart prices={coin.sparkline_in_7d?.price??[]} /></div>
                    <div className="flex justify-center">
                      <button onClick={() => toggleWatchlist(coin.id)}
                        className="p-1 rounded transition-colors" aria-label={watchlist.includes(coin.id)?'Remove from watchlist':'Add to watchlist'}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t" style={{ borderColor:'var(--border)', background:'var(--surface-2)' }}>
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(p=>Math.max(1,p-1))} disabled={page===1}
                  ...
                  style={{ color:'var(--text-muted)' }} aria-label="Previous page">
                  <ChevronLeft size={16}/>
                </button>
                <span className="text-xs num px-2" style={{ color:'var(--text)' }}>{page} / {totalPages}</span>
                <button onClick={() => setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
                  className="p-1.5 rounded-lg disabled:opacity-30 hover:opacity-70 transition-opacity"
                  style={{ color:'var(--text-muted)' }} aria-label="Next page">
        ...
        <p className="text-center text-xs mt-6" style={{ color:'var(--text-faint)' }}>
          Data by <a href="https://www.coingecko.com" target="_blank" rel="noopener noreferrer" style={{ color:'var(--primary)' }}>CoinGecko</a>
